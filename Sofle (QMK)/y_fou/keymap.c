...
#define SPC_NBSC LT(_NUMBER_SHORTCUT, KC_SPC)
#define BSPC_SYM LT(_SYMBOL, KC_BSPC)
#define DOT_NBSC LT(_NUMBER_SHORTCUT, KC_DOT)
#define ENT_NAV LT(_NAVIGATION, KC_ENT)
#define QUOT_UML LT(_UMLAUT, DE_QUOT)

#define QWERTZ TO(_QWERTZ)
#define GREEK TO(_GREEK)
#define YFOU TO(_YFOU)

#define LSH_σ LSFT_T(UC(0x03c3))
#define RSH_α RSFT_T(UC(0x03b1))


// Modifier+Key Overrides: specify overrides as consts, then add them to the array bellow
const key_override_t delete_key_override            = ko_make_basic(MOD_MASK_SHIFT, BSPC_SYM, KC_DEL);
...
// combos
const uint16_t PROGMEM combo_adia[] = {QUOT_UML, RSH_A, COMBO_END};
const uint16_t PROGMEM combo_odia[] = {QUOT_UML, RAL_O, COMBO_END};
const uint16_t PROGMEM combo_udia[] = {QUOT_UML, KC_U, COMBO_END};
const uint16_t PROGMEM combo_ss[]   = {QUOT_UML, LSH_S, COMBO_END};
const uint16_t PROGMEM combo_context2[] = {QUOT_UML, CONTXT, COMBO_END};
const uint16_t PROGMEM combo_context2b[] = {ENT_NAV, CONTXT, COMBO_END};
combo_t key_combos[] = {
    COMBO(combo_adia, DE_ADIA),
    COMBO(combo_odia, DE_ODIA),
    COMBO(combo_udia, DE_UDIA),
    COMBO(combo_ss, DE_SS),
    COMBO(combo_context2, CONTXT2),
    COMBO(combo_context2b, CONTXT2),
};
 ...
 * `-------------+------+------+------|'',------.                      ,--+---.''|------+------+------+------+------'
 *               |QWERTZ|      |  Tab | /Space /,-----.          ,-----.\ Enter\ |   '  |   +  |   /  |
 *               '----------------------------'/ Bspc /          \   .  \'----------------------------'
 *                           +LeftNum +Nmb/Sct'------'            '------' +Navi  +Umlaut
 *                                             +Symbl          +Numb/Shtct      */
[_YFOU] = LAYOUT(
  ...
  KC_ESC, DE_Z,   CONTXT, KC_W,   KC_G,   KC_J,   KC_MUTE,     XXXXXX,  KC_P,   KC_H,   DE_MINS,DE_COMM,CONTXT, XXXXXX,
                  QWERTZ, XXXXXX, TAB_NUM,SPC_NBSC,BSPC_SYM,   DOT_NBSC,ENT_NAV,QUOT_UML,DE_PLUS,DE_SLSH
),


/* SYMBOL
 * ,-----------------------------------------.                            ,-----------------------------------------.
 * |      |  F21 |  F22 |  F23 |  F24 |      |                            |  F16 |  F17 |  F18 |  F19 |  F20 |  (`) | <- dead
 * |------+------+------+------+------+------|                            |------+------+------+------+------+------|
 * |      |   @  |   (  |   )  |   "  |   '  |                            |   ]  |   [  |   *  |   /  |   \  |  (´) | <- dead
 * |------+------+------+------+------+------|                            |------+------+------+------+------+------|
 * |Repeat|   ~  |   <  |   -  |   =  |   >  |  ,----.            ,----.  |   ;  |   =  |   :  |   !  |   ?  |Repeat|
 ...
 *                                            '------'            '------'      */
[_SYMBOL] = LAYOUT(
  XXXXXX, KC_F21, KC_F22, KC_F23, KC_F24, XXXXXX,                       KC_F16, KC_F17, KC_F18, KC_F19, KC_F20, DE_GRV,
  XXXXXX, DE_AT,  DE_LPRN,DE_RPRN,DE_DQUO,DE_QUOT,                      DE_RBRC,DE_LBRC,DE_ASTR,DE_SLSH,DE_BSLS,DE_ACUT,
  ______, DE_TILD,DE_LABK,DE_MINS,DE_EQL, DE_RABK,                      DE_SCLN,DE_EQL, DE_COLN,DE_EXLM,DE_QUES,QK_REP,
  CW_TOGG,DE_PIPE,DE_AMPR,DE_HASH,DE_PERC,DE_DLR, KC_MUTE,     XXXXXX,  DE_RCBR,DE_LCBR,DE_PLUS,DE_MINS,CIRCUM, DE_CIRC,
                  DE_MICR,FTICK,  DE_DEG, BTICK,  XXXXXX,      KC_SPC,  KC_ENT, DE_UNDS,DE_SUP2,DE_SUP3
),
 ...
 * |      |Cdeblk|Cdelne|      |      |      |                            |   /  |   *  |   8  |   9  |   ,  |   $  |
 * |------+------+------+------+------+------|                            |------+------+------+------+------+------|
 * |Repeat|      |      | Space| Find | Save |  ,----.            ,----.  |   +  |   0  |   1  |   2  |   3  |Repeat|
 * |------+------+------+------+------+------| ( MUTE )          (DTTPRT) |------+------+------+------+------+------|
 * |CapsW | Undo | Redo |  Cut | Copy | Paste|  `----´            `----´  |   -  |   4  |   5  |   6  |   7  |   €  |
 * `-------------+------+------+------|'',------.                      ,--+---.''|------+------+------+------+------'
 *               |      |select| Enter| / Space/,-----.         ,-----.\  Enter\ |   .  |   ²  |   ³  |
 *               '---------all----------------'/  Del /          \   .  \'----------------------------'
 *                                            '------'            '------'      */
[_NUMBER_SHORTCUT] = LAYOUT(
  XXXXXX, KC_F11, KC_F12, KC_F13, KC_F14, KC_F15,                       KC_F6,  KC_F7,  KC_F8,  KC_F9,  KC_F10, XXXXXX,
  XXXXXX, CDEBLCK,CDELINE,XXXXXX, XXXXXX, XXXXXX,                       KC_PSLS,KC_PAST,KC_8,   KC_9,   DE_COMM,DE_DLR,
  QK_REP, XXXXXX, XXXXXX, KC_SPC, FIND,   SAVE,                         KC_PPLS,KC_0,   KC_1,   KC_2,   KC_3,   QK_REP,
  CW_TOGG,UNDO,   REDO,   CUT,    COPY,   PASTE,  KC_MUTE,     XXXXXX,  KC_PMNS,KC_4,   KC_5,   KC_6,   KC_7,   DE_EURO,
                  XXXXXX, SELALL, KC_ENT, KC_SPC, KC_DEL,      DE_DOT,  KC_ENT, DE_DOT, DE_SUP2,DE_SUP3
),
 ...
 * |------+------+------+------+------+------| ( MUTE )          (DTTPRT) |------+------+------+------+------+------|
 * |      |      |Cntxt2|      |      |      |  `----´            `----´  |      | Enter| Bckwd|  Fwd |Cntxt2| Play/Pause|
 * `-------------+------+------+------|'',------.                      ,--+---.''|------+------+------+------+------'
 *               |      |      |  Tab | /Space /,-----.          ,-----.\      \ |      |      |      |
 *               '----------------------------'/      /          \      \'----------------------------'
 *                                            '------'            '------'      */
 [_NAVIGATION] = LAYOUT(
  XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX,                       XXXXXX, XXXXXX, KC_CALC,XXXXXX, XXXXXX, KC_PSCR,
  ...
  XXXXXX, XXXXXX, CONTXT2,XXXXXX, XXXXXX, XXXXXX, XXXXXX,       XXXXXX, XXXXXX, KC_ENT, KC_WBAK,KC_WFWD,CONTXT2,KC_PAUS,
 ...
 *                                            '------'            '------'      */
 [_LNUMBER] = LAYOUT(
  ...
  DE_DLR, DE_COMM,KC_9,   KC_8,   KC_PAST,KC_PSLS,                      XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX,
  QK_REP, KC_3,   KC_2,   KC_1,   KC_0,   KC_PPLS,                      XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX,
  DE_EURO,KC_7,   KC_6,   KC_5,   KC_4,   KC_PMNS,XXXXXX,       XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX, XXXXXX,
                  XXXXXX, DE_COMM,XXXXXX, DE_DOT, KC_DEL,       KC_SPC, XXXXXX, XXXXXX, XXXXXX, XXXXXX
 ),
 ...
 *                                            '------'            '------'      */
 [_QWERTZ] = LAYOUT(
  ...
  KC_LCTL,DE_Y,   KC_X,   KC_C,   KC_V,   KC_B,   XXXXXX,       XXXXXX, KC_N,   KC_M,   DE_COMM,DE_DOT, DE_MINS,DE_PLUS,
                  GREEK,  DE_LABK,KC_LALT,KC_SPC, KC_BSPC,      KC_DEL, KC_ENT, DE_ACUT,DE_HASH,DE_CIRC
 ),
 ...
 * |      |      |   μ  |   λ  |   χ  |   κ  |                            |   ξ  |   φ  |   ο  |   ψ  |   θ  |      |
 * |------+------+------+------+------+------|                            |------+------+------+------+------+------|
 * |Repeat|   σ  |   τ  |   ρ  |   δ  |   υ  |  ,----.            ,----.  |   β  |   ν  |   ε  |   ι  |   α  |Repeat|
 * |------+------+------+------+------+------| ( MUTE )          (DTTPRT) |------+------+------+------+------+------|
 * |      |   ζ  |Contxt|   ω  |   γ  |      |  `----´            `----´  |   π  |   η  |   -  |   ,  |Contxt|      |
 * `-------------+------+------+------|'',------.                      ,--+---.''|------+------+------+------+------'
 *               |      |      |      | /      /,-----.          ,-----.\      \ |      |      |      |
 *               '----------------------------'/      /          \      \'----------------------------'
 *                                            '------'            '------'      */
 [_GREEK] = LAYOUT(
  ______, ______, ______, ______, ______, ______,                       ______, ______, ______, ______, ______, ______,
  ______, ______,UC(0x03bc),UC(0x03bb),UC(0x03c7),UC(0x03ba),         UC(0x03be),UC(0x03c6),UC(0x03bf),UC(0x03c8),UC(0x03b8), ______,
 ______,LSH_σ,UC(0x03c4),UC(0x03c1),UC(0x03b4),UC(0x03c5),            UC(0x03b2),UC(0x03bd),UC(0x03b5),UC(0x03b9),RSH_α,______,
 ______,UC(0x03b6),______,UC(0x03c9),UC(0x03b3),______,______, ______,UC(0x03c0),UC(0x03b7),______,______,______,______,
                  YFOU,  ______, ______, ______, ______,       ______, ______, ______, ______, ______
 ),
...
static void process_context2(uint16_t keycode, uint8_t mods){
    switch (keycode) {
        ...
        case KC_H: tap_code16(KC_O); break; // for "Pho (photo, phone)"
        case KC_N: tap_code16(KC_H); break;
        case KC_T: tap_code16(KC_V); break;
        case KC_I: tap_code16(KC_Q); break;
        case LCT_R: tap_code16(KC_W); break;
        case KC_K: tap_code16(KC_G); break;
        case KC_P: tap_code16(KC_H); break; // for "Pho (photo, phone)"
    }
}
...
uint16_t get_alt_repeat_key_keycode_user(uint16_t keycode, uint8_t mods) {
    if ((mods & MOD_MASK_CTRL)) {  // Was Ctrl held?
        ...
        switch (keycode) {
            ...
            case RGU_B: return KC_H;
            case DE_UDIA: return KC_H;
        }
    }
